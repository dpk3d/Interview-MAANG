...
# Time Complexity :- O(N)
def max_profit_dp_way(arr):
    length = len(arr)
    max_price = arr[length - 1]  # till here only one transaction
    min_price = arr[0]
    max_profit = [0] * length
    for x in range(length - 2, 0, -1):
        if arr[x] > max_price:
            max_price = arr[x]
        max_profit[x] = max(max_profit[x + 1], max_price - arr[x])
    for x in range(1, length):
        if arr[x] < min_price:
            min_price = arr[x]
        max_profit[x] = max(max_profit[x - 1], max_profit[x] + (arr[x] - min_price))
    return max_profit[length - 1]


arr = [10, 22, 5, 75, 65, 80]
print("Maximum Profit : ", max_profit_dp_way(arr))
# Output: Maximum Profit :  87


def max_profit(price_array):
    if not price_array or len(price_array) == 1:
        return 0
    profit = 0
    for x in range(1, len(price_array)):
        if price_array[x] > price_array[x - 1]:
            profit += price_array[x] - price_array[x - 1]
...
# Output: Maximum Profit :  72


def max_profit_short(arr):
    ...
    for x in range(1, len(arr)):
        max_profit_list.append(max(0, arr[x] - arr[x - 1]))
